...
export function computeFxTranslation(
  ...
  const cta = diff; // positive CTA if needed to balance (direction depends on sign convention)
  ...
  return {
    ...
    holdings,
    translatedByType,
    cta: cta.abs(), // CTA is conventionally the absolute plug booked to equity
    balancedWithCta,
  };
}
