...
The package is at v0.11.0 and fully packable with `npm pack`. For library consumption in another repo (Money + Ledger kernel + layers):

```bash
# Option 1: packed tarball (recommended for exact version)
cd /path/to/ledger && npm pack
npm install /path/to/ledger-0.11.0.tgz
